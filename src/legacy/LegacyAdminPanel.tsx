...
type FormaAtendimento = {
  ...
  estrutura?: ConvenioEstrutura;
  feegow_convenio_id?: number | null;
};

type ConvenioEstrutura =
  | "somente_convenio"
  | "convenio_plano"
  | "produto_plano"
  | "rede_plano";

type ConvenioCatalogoItem = {
  feegow_convenio_id: number | null;
  feegow_nome: string | null;
  registro_ans?: string | null;
  ativo_feegow: boolean;
  planos_feegow: number;
...
type ConvenioCatalogoResponse = {
  items: ConvenioCatalogoItem[];
};

type FeegowPlanoCatalogo = {
...
type AdminUser = {
  ...
  tipoUsuario?: "super_admin" | "cliente_admin" | "cliente_operador";
  ativo: boolean;
  mfa_enabled: boolean;
...
function convenioEstruturaLabel(value: ConvenioEstrutura) {
  const labels: Record<ConvenioEstrutura, string> = {
    somente_convenio: "Somente convênio",
    convenio_plano: "Convênio + plano",
    produto_plano: "Convênio + produto + plano",
    rede_plano: "Convênio + rede + plano",
  };
  return labels[value];
}
...
export default function LegacyAdminPanel() {
  ...
  const conveniosFeegowDisponiveis = useMemo(
    () =>
      convenioCatalogo
        .filter((item) => !item.registrado && item.ativo_feegow)
        .sort((a, b) => a.nome_exibicao.localeCompare(b.nome_exibicao, "pt-BR")),
    [convenioCatalogo],
  );

  const selectedNovoConvenioFeegow = useMemo(
    () =>
      conveniosFeegowDisponiveis.find(
        (item) =>
          Number(item.feegow_convenio_id) ===
          Number(novoConvenioCatalogo.feegow_convenio_id),
      ) || null,
  ...
  const produtosFiltrados = useMemo(() => {
    ...
    return produtos.filter((produto) => {
      ...
      return [
        produto.nome,
        produto.tipo,
        produto.codigo_operadora,
        produto.acomodacao_ou_uf,
        produto.codigo_produto,
        produto.plano_nome,
        produto.rede_nome,
  ...
  async function selecionarClienteParaMedicos(rawClienteId: number) {
    ...
    try {
      await Promise.all([
        ...
        loadConvenioCatalogo(clienteId),
  ...
  function printDashboardPdf() {
    ...
    if (dashboardReportSections.logs) sections.push(`<h3>Logs recentes</h3><table><thead><tr><th>Data</th><th>Clínica</th><th>Telefone</th><th>Estado da confirmação</th><th>Etapa</th><th>Intenção</th></tr></thead><tbody>${dashboardGerencial.reportRows.slice(0, 80).map((row) => `<tr><td>${formatDateTimeShort(row.data)}</td><td>${row.cliente}</td><td>${row.telefone}</td><td>${row.status}</td><td>${row.etapa}</td><td>${row.intencao}</td></tr>`).join("")}</tbody></table>`);
  ...
  useEffect(() => {
    ...
    const medicoValido = selectedMedicoId
      ? medicos.find(
          (medico) =>
            Number(medico.id) === Number(selectedMedicoId) &&
            Number(medico.cliente_id) === Number(selectedClienteId),
        )
      : null;
  ...
  async function cadastrarConvenioCatalogo(event: React.FormEvent) {
    event.preventDefault();
    if (!selectedClienteId || !selectedNovoConvenioFeegow?.feegow_convenio_id) {
      showToast("Selecione um convênio disponível no catálogo Feegow", true);
      return;
    }
    ...
    try {
      const created = await api<FormaAtendimento & { estrutura: ConvenioEstrutura }>(
        ...
        body: JSON.stringify({
          ...
          nome_exibicao:
            novoConvenioCatalogo.nome_exibicao.trim() ||
            selectedNovoConvenioFeegow.feegow_nome ||
  ...
  function convenioCatalogoToForma(item: ConvenioCatalogoItem): FormaAtendimento | null {
    if (!item.forma_id || !item.convenio_id) return null;

    return {
      id: item.forma_id,
      cliente_id: selectedClienteId || 0,
      tipo: "convenio",
      nome: item.nome_exibicao,
      convenio_id: item.convenio_id,
      cliente_convenio_id: item.cliente_convenio_id,
      exige_plano: item.exige_plano,
      permite_agendamento_online: item.permite_agendamento_online,
      ativo: item.ativo,
      convenio_global: item.nome_oficial,
      estrutura: item.estrutura,
      feegow_convenio_id: item.feegow_convenio_id,
    };
  }
  ...
  async function criarProduto(event: React.FormEvent) {
    ...
    try {
      const estrutura =
        selectedForma.estrutura ||
        (selectedForma.exige_plano ? "convenio_plano" : "somente_convenio");
      const nome =
        estrutura === "produto_plano"
          ? `${novoProduto.codigo_produto.trim()} - ${novoProduto.plano_nome.trim()}`
          : estrutura === "rede_plano"
            ? `${novoProduto.rede_nome.trim()} - ${novoProduto.plano_nome.trim()}`
            : novoProduto.plano_nome.trim();
          ...
          tipo:
            estrutura === "produto_plano"
              ...
              : estrutura === "rede_plano"
                ? "rede_plano"
                : "plano",
          codigo_operadora: novoProduto.codigo_produto || null,
          acomodacao_ou_uf: novoProduto.rede_nome || null,
          codigo_produto: novoProduto.codigo_produto || null,
          plano_nome: novoProduto.plano_nome || null,
          rede_nome: novoProduto.rede_nome || null,
          feegow_plano_id: novoProduto.feegow_plano_id
            ? Number(novoProduto.feegow_plano_id)
            : null,
    ...
    <div className="app">
      <aside className="sidebar">
        <div className="sidebarProfile">
          <h1>AgendAI</h1>
          <p>{authUser.tipoUsuario === "cliente_operador" ? "Atendente" : authUser.tipoUsuario === "cliente_admin" ? "Gestor assistencial" : "Administrador técnico"}</p>
      ...
      <main className="content">
        <header className="header">
          <div>
            <h2>Painel Administrativo Angel</h2>
            <p>Policlínica Bonfiglioli · operação, confirmações e cobertura de agendamento.</p>
          </div>
        ...
        {activeTab === "medicos" && (
          <>
            <section className="coverageHub">
              <div className="coverageModuleHeader">
                <div>
                  <span className="coverageEyebrow">GESTÃO ASSISTENCIAL</span>
                  <h3>Convênios e aceites</h3>
                  <p>
                    Cadastre a estrutura comercial da Polibon e defina quais combinações
                    podem ser oferecidas no WhatsApp.
                  </p>
                </div>
                <span className="coverageReadOnlyBadge">
                  {canManageCoverage ? "Gestão operacional" : "Somente leitura"}
                </span>
              </div>

              <div className="coverageSectionTabs" role="tablist" aria-label="Convênios e aceites">
                <button
                  type="button"
                  className={coverageSection === "catalogo" ? "active" : ""}
                  onClick={() => setCoverageSection("catalogo")}
                >
                  Catálogo de convênios
                </button>
                <button
                  type="button"
                  className={coverageSection === "aceites" ? "active" : ""}
                  onClick={() => setCoverageSection("aceites")}
                >
                  Coberturas e aceites
                </button>
              </div>
            </section>
              ...
              <section className="coverageCatalog">
                ...
                {canManageCoverage && (
                  <details className="coverageAvailablePanel">
                    ...
                    <div className="coverageCatalogCreateGrid">
                      <label>
                        Convênio no Feegow
                        <select
                          value={novoConvenioCatalogo.feegow_convenio_id}
                          onChange={(event) => {
                            const item = conveniosFeegowDisponiveis.find(
                              (candidate) =>
                                Number(candidate.feegow_convenio_id) ===
                                Number(event.target.value),
                            );
                            setNovoConvenioCatalogo((current) => ({
                              ...current,
                        ...
                        >
                          ...
                          {conveniosFeegowDisponiveis.map((item) => (
                            <option
                              key={item.feegow_convenio_id || item.nome_exibicao}
                              value={item.feegow_convenio_id || ""}
                            >
                              {item.feegow_nome || item.nome_exibicao} · {item.planos_feegow} plano(s)
                            </option>
                          ))}
                        </select>
                      </label>
                      ...
                      <label>
                        Nome exibido
                        <input
                          value={novoConvenioCatalogo.nome_exibicao}
                          onChange={(event) =>
                            setNovoConvenioCatalogo((current) => ({
                              ...current,
                          ...
                          placeholder="Nome que aparecerá no administrativo"
                          required
                        />
                      </label>
                      ...
                      <label>
                        Estrutura comercial
                        <select
                          value={novoConvenioCatalogo.estrutura}
                          onChange={(event) =>
                            setNovoConvenioCatalogo((current) => ({
                              ...current,
                              estrutura: event.target.value as ConvenioEstrutura,
                            }))
                          }
                        >
                          <option value="somente_convenio">Somente convênio</option>
                          <option value="convenio_plano">Convênio + plano</option>
                          <option value="produto_plano">Convênio + produto + plano</option>
                          <option value="rede_plano">Convênio + rede + plano</option>
                        </select>
                      </label>
                      ...
                      <button
                        type="submit"
                        disabled={
                          loadingAction === "convenio.catalogo" ||
                          !selectedNovoConvenioFeegow
                        }
                      >
                        {loadingAction === "convenio.catalogo"
                          ? "Cadastrando..."
                  ...
                  <section className="coverageProductPanel" id="coverage-product-panel">
                    <div className="coverageProductHeader">
                      <div>
                        ...
                        <h4>{selectedForma.nome}</h4>
                        <p>
                          {convenioEstruturaLabel(
                            selectedForma.estrutura ||
                              (selectedForma.exige_plano
                                ? "convenio_plano"
                                : "somente_convenio"),
                          )}
                        </p>
                    ...
                    {selectedForma.estrutura !== "somente_convenio" && (
                      <form className="coverageProductCreate" onSubmit={criarProduto}>
                        {selectedForma.estrutura === "produto_plano" && (
                          <label>
                            Produto / código
                            <input
                        ...
                        {selectedForma.estrutura === "rede_plano" && (
                          <label>
                            Rede
                            <input
                              value={novoProduto.rede_nome}
                              onChange={(event) =>
                                setNovoProduto((current) => ({
                                  ...current,
                                  rede_nome: event.target.value,
                                }))
                              }
                        ...
                        <label>
                          Plano
                          <input
                            value={novoProduto.plano_nome}
                            onChange={(event) =>
                              setNovoProduto((current) => ({
                                ...current,
                                plano_nome: event.target.value,
                              }))
                            }
                    ...
                    <div className="coverageCatalogFilters">
                      <label>
                        ...
                        <input
                          ...
                          placeholder="Plano, produto, rede, código ou ID Feegow"
                        />
                      </label>
                      <label>
                        Tipo
                        <select
                          value={produtoTipoFiltro}
                          onChange={(event) => {
                            setProdutoTipoFiltro(event.target.value);
                            setProdutoPage(1);
                          }}
                        >
                          <option value="todos">Todos</option>
                    ...
                    <PaginationBar
                      ...
                      label="Itens do convênio"
                      onPrev={() => setProdutoPage((page) => Math.max(1, page - 1))}
                      onNext={() =>
                        setProdutoPage((page) => Math.min(totalProdutoPages, page + 1))
                      }
                    />
            ...
            {coverageSection === "aceites" && (
              <>
                ...
                doctors={medicos.map((medico) => ({
                  id: medico.id,
                  name: medico.nome,
                  registry: medico.registro_profissional,
                  specialties: medico.especialidades,
                  active: medico.ativo,
                }))}
                coverages={aceites.map((aceite) => ({
                  id: aceite.id,
                  doctorId: aceite.medico_id,
                  doctor: aceite.medico,
                  specialtyId: aceite.especialidade_id,
                  specialty: aceite.especialidade,
                  insurer: aceite.convenio,
                  insurerGloballyActive: aceite.convenio_ativo_global !== false,
                  product: aceite.produto,
                  productType: aceite.produto_tipo,
                  operatorCode: aceite.codigo_operadora,
                  networkOrAccommodation: aceite.acomodacao_ou_uf,
                  active: aceite.ativo,
                  ruleSource: aceite.origem_regra || aceite.fonte_tipo,
                  ruleNote: aceite.observacao_regra,
                  createdAt: aceite.created_at,
                ...
                updatingCoverageId={
                  loadingAction?.startsWith("aceite-")
                    ? Number(loadingAction.replace("aceite-", ""))
                    : null
                }
                onSelectDoctor={(doctorId) => {
                  ...
                  setAceites([]);
                  setCoverageLoading(false);
                  setSelectedMedicoId(doctorId);
                }}
                onToggleCoverage={(coverage) => {
                  const matchingAceite = aceites.find(
                    (aceite) => Number(aceite.id) === Number(coverage.id),
                  );
        ...
        {legacyCoverageEditorEnabled && activeTab === "medicos" && !medicosClienteSelecionado && (
        ...
        {activeTab === "operacao_tempo_real" && (
          <OperationRealtimePanel
            apiRequest={api}
            onOpenIndicators={() => setActiveTab("visao_dia")}
          />
        )}
            ...
            <section className="card managementDashboardCard">
              ...
              <div className="dashboardFiltersGrid dashboardFiltersGridCompanyOnly">
                ...
                <label>
                  ...
                  >
                    <option value="todos">Polibon</option>
        ...
        {activeTab === "followups" && (
          <section className="card followupOperationalPanel">
            <div className="sectionHeader dashboardHeaderRow">
              <div>
                <h3>Confirmações de consulta</h3>
                <p>Acompanhe consultas com confirmação programada, envios, respostas e solicitações do paciente.</p>
              </div>
              <div className="headerActions">
                <button type="button" onClick={() => loadFollowupsPanel()} disabled={followupsLoading}>
                  {followupsLoading ? "Atualizando..." : "Atualizar confirmações"}
                </button>
              </div>
            </div>
            ...
            {!followupsPanel && <div className="emptyStateBox">Clique em Atualizar confirmações para carregar a fila operacional.</div>}

            {followupsPanel && (
              <>
                <div className="followupSummaryGrid">
                  <div className="followupSummaryCard"><strong>{followupsPanel.summary.total}</strong><span>Confirmações programadas</span></div>
                  <div className="followupSummaryCard warning"><strong>{followupsPanel.summary.pending}</strong><span>Aguardando envio</span></div>
                  <div className="followupSummaryCard info"><strong>{followupsPanel.summary.semResposta}</strong><span>Aguardando resposta</span></div>
                  ...
                  <table>
                    ...
                    <tbody>
                      {followupsPanel.items.length ? followupsPanel.items.map((item) => (
                        ...
                        <tr><td colSpan={8}>Nenhuma confirmação encontrada para os filtros.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
