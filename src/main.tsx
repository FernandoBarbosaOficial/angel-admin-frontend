...
function TopListPanel({ title, rows }: { title: string; rows: DashboardTopItem[] }) {
  ...
  return (
    <div className="dashboardPanel topListPanel">
      <h4>{title}</h4>
      {rows.length === 0 ? (
        <p className="mutedText">Sem dados no perÃ­odo.</p>
      ) : (
        ...
        <div className="auditHumanList">
          {rows.map((row) => (
            <div className="auditHumanItem" key={row.key}>
              ...
              <span>
                {row.inicio} ÃƒÂ s {row.fim} Â· intervalo de {row.intervalo}
              </span>
            </div>
          ))}
        </div>
...
function App() {
  ...
  const scopedClientes = useMemo(() => {
    ...
    // Em perfis de clÃ­nica, a API jÃ¡ retorna apenas os clientes vinculados.
    // Se o token antigo/localStorage ainda nÃ£o trouxer clienteIds, usamos a lista carregada como fonte visual.
  ...
  const accessScopeDetail = isGlobalAdmin
    ? "Pode cadastrar clientes, canais WhatsApp, usuÃ¡rios e operar o dashboard global."
    : linkedClientesCount === 0
      ? "Nenhuma clÃ­nica vinculada ao seu usuÃ¡rio."
      : operationalClientesCount === 0
        ? `${linkedClientesCount} clÃ­nica(s) vinculada(s), mas nenhuma operacional. ${inactiveLinkedClientes[0]?.nome_fantasia || "ClÃ­nica"} estÃ¡ inativa.`
        : `${linkedClientesCount} clÃ­nica(s) vinculada(s), ${operationalClientesCount} operacional(is).`;
  ...
  function dashboardFilterLabel() {
    ...
    return `${days} dia(s) Â· ${cliente} Â· ${grupo}`;
  }
  ...
  function makeDashboardWorkbook() {
    ...
    if (dashboardReportSections.resumo) {
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet([
        ...
        { Indicador: "Pacientes Ãºnicos", Valor: dashboardGerencial.summary.pacientesUnicos },
        ...
        { Indicador: "Agendamentos concluÃ­dos", Valor: dashboardGerencial.summary.agendamentosConcluidos },
        { Indicador: "Abandonadas", Valor: dashboardGerencial.summary.conversasAbandonadas },
        { Indicador: "Taxa de conversÃ£o", Valor: `${dashboardGerencial.summary.taxaConversao}%` },
  ...
  function exportDashboardExcel() {
    try {
      ...
      showToast(error instanceof Error ? `âŒ ${error.message}` : "âŒ Erro ao gerar Excel", true);
    }
  }

  function topListHtml(title: string, rows: DashboardTopItem[]) {
    if (!rows.length) return `<h3>${title}</h3><p>Sem dados no perÃ­odo.</p>`;
  ...
  function printDashboardPdf() {
    if (!dashboardGerencial) {
      showToast("âŒ Carregue o dashboard antes de gerar PDF", true);
      return;
    }
    ...
    if (dashboardReportSections.resumo) sections.push(`
      ...
      <div class="kpis">
        ...
        <div><strong>${dashboardGerencial.summary.pacientesUnicos}</strong><span>Pacientes Ãºnicos</span></div>
        <div><strong>${dashboardGerencial.summary.agendamentosIniciados}</strong><span>Agendamentos iniciados</span></div>
        <div><strong>${dashboardGerencial.summary.agendamentosConcluidos}</strong><span>Agendamentos concluÃ­dos</span></div>
        <div><strong>${dashboardGerencial.summary.taxaConversao}%</strong><span>ConversÃ£o estimada</span></div>
      </div>`);
    if (dashboardReportSections.evolucao) sections.push(topListHtml("EvoluÃ§Ã£o por dia", dashboardGerencial.breakdowns.byDay.map((row) => ({ label: row.label, total: row.mensagens }))));
    ...
    if (dashboardReportSections.convenios) sections.push(topListHtml("ConvÃªnios / planos", dashboardGerencial.breakdowns.byConvenioPlano));
    if (dashboardReportSections.etapas) sections.push(topListHtml("Etapas do fluxo", dashboardGerencial.breakdowns.byStage));
    if (dashboardReportSections.logs) sections.push(`<h3>Logs recentes</h3><table><thead><tr><th>Data</th><th>Cliente</th><th>Telefone</th><th>Status</th><th>Etapa</th><th>IntenÃ§Ã£o</th></tr></thead><tbody>${dashboardGerencial.reportRows.slice(0, 80).map((row) => `<tr><td>${formatDateTimeShort(row.data)}</td><td>${row.cliente}</td><td>${row.telefone}</td><td>${row.status}</td><td>${row.etapa}</td><td>${row.intencao}</td></tr>`).join("")}</tbody></table>`);

    const win = window.open("", "_blank", "width=1100,height=800");
    if (!win) {
      showToast("âŒ Pop-up bloqueado. Libere pop-ups para gerar PDF.", true);
      return;
    }
    win.document.write(`<!doctype html><html><head><title>RelatÃ³rio gerencial Angel</title><style>
    ...
    </style></head><body><button onclick="window.print()">Salvar/imprimir PDF</button><h1>RelatÃ³rio gerencial Angel</h1><div class="meta">${dashboardFilterLabel()} Â· Gerado em ${formatDateTimeShort(dashboardGerencial.generatedAt)}</div>${sections.join("")}</body></html>`);
    win.document.close();
    win.focus();
  }
  ...
  const whatsappActiveBlocks = useMemo(() => {
    ...
    // Eventos histÃ³ricos com blockedUntil futuro nÃ£o devem recriar uma linha ativa
    ...
    <div className="app">
      <aside className="sidebar">
        ...
        <nav>
          ...
          >
            {isGlobalAdmin ? "Clientes" : "Minhas clÃ­nicas"}
          </button>
          <button
      ...
      <main className="content">
          ...
          <div className="accessScopeStats">
            <span>
              {isGlobalAdmin
                ...
                : `${linkedClientesCount} vinculada(s) Â· ${operationalClientesCount} operacional(is)`}
            </span>
            {!isGlobalAdmin && linkedClientesCount === 0 && <strong className="criticalText">Nenhuma clÃ­nica vinculada</strong>}
            {!isGlobalAdmin && linkedClientesCount > 0 && operationalClientesCount === 0 && (
              <strong className="warningText">ClÃ­nica vinculada, mas inativa</strong>
            )}
          </div>
        </section>
        ...
        {activeTab === "medicos" && !selectedCliente && (
          <section className="card full selectionRequiredCard">
            <div className="selectionRequiredIcon">Ã°Å¸ÂÂ¥</div>
            <div>
            ...
            <div className="card">
              ...
              <form onSubmit={salvarMedico} className="medicoForm">
                ...
                <input
                  ...
                  placeholder="Dias/horÃ¡rios. Ex.: 2Ã‚Âª e 4Ã‚Âª M"
                />
                <input
                ...
                {medicosFiltrados.map((medico) => (
                  ...
                  >
                    ...
                    {(medico.dias || medico.andar) && (
                      <span>
                        {[medico.dias, medico.andar].filter(Boolean).join(" â€Â¢ ")}
                      </span>
                    )}
            ...
            <section className="card">
              ...
              <div className="groupOverviewPanel">
                <div className="groupOverviewHeader">
                  <div>
                    <span className="sectionEyebrow">Multi-clÃ­nica agrupada</span>
                    <h4>Grupos e unidades derivados dos canais</h4>
                    <p>Visualize redes que usam um WhatsApp Ãºnico com menu de escolha de unidade, sem alterar a estrutura atual do banco.</p>
                  </div>
                ...
                {whatsappGruposUnidades ? (
                  <>
                    <div className="groupOverviewSummary">
                      ...
                      <div><strong>{whatsappGruposUnidades.summary.gruposCriticos}</strong><span>CrÃ­ticos</span></div>
                        ...
                        {whatsappGruposOrdenados.map((grupo) => {
                          ...
                          return (
                            <article key={grupo.grupoId} className={`groupOverviewItem ${grupo.status}`}>
                              ...
                              >
                                <span className="conversationChevron">{expanded ? "â–¾" : "â–¸"}</span>
                                <div>
                                  <strong>{grupo.nomeGrupo}</strong>
                                  <span>{grupo.canalNome} Â· {grupo.phoneNumberId || "sem phone_number_id"} Â· {grupo.totalUnidades} unidade(s)</span>
                                </div>
                                <span className={`severityBadge ${grupo.status === "pronto" ? "success" : grupo.status === "atencao" ? "warning" : "critical"}`}>
                                  {grupo.status === "pronto" ? "PRONTO" : grupo.status === "atencao" ? "ATENÃ‡ÃƒO" : "CRÃTICO"}
                                </span>
                              </button>
                                ...
                                <div className="groupAlertRow">
                                  {grupo.alerts.slice(0, 2).map((alert, index) => (
                                    <span key={`${grupo.grupoId}-alert-${index}`} className={alert.severity === "critical" ? "dangerText" : "warningText"}>
                                      {alert.severity === "critical" ? "ðŸš¨" : "âš ï¸"} {alert.message}
                                    </span>
                                  ))}
                                ...
                                <div className="groupUnitsGrid">
                                  {grupo.unidades.map((unidade) => (
                                    <div key={`${grupo.grupoId}-${unidade.id}`} className={`groupUnitCard ${unidade.operacional ? "ok" : "critical"}`}>
                                      ...
                                      <span>{unidade.papel === "base" ? "Cliente base" : "Unidade"} Â· {unidade.status || "sem status"}</span>
                                      <small>{unidade.operacional ? "Operacional" : "Inativa / fora de operaÃ§Ã£o"}</small>
                                      {unidade.duplicadoEmOutrosGrupos && <small className="warningText">TambÃ©m vinculada a outro grupo</small>}
                                    </div>
                                  ))}
                                </div>
                              )}
                  ...
                  <div className="emptyState">Atualize os grupos para visualizar a estrutura multi-clÃ­nica.</div>
                )}
              </div>
            ...
            <section className="card managementDashboardCard">
              <div className="sectionHeader dashboardHeaderRow">
                <div>
                  ...
                  <p>VisÃ£o comercial por grupo, unidade, especialidade e canal. Use os filtros para escolher a empresa/unidade, montar o dashboard e exportar o mesmo recorte em Excel ou PDF.</p>
                </div>
                <div className="headerActions">
              ...
              <div className="dashboardFiltersGrid">
                ...
                <label>
                  ...
                  <select value={dashboardGerencialFilters.groupId} onChange={(event) => setDashboardGerencialFilters((current) => ({ ...current, groupId: event.target.value }))}>
                    ...
                    {((dashboardGerencial?.groups || []).map((grupo) => ({ id: grupo.id, nome: grupo.nome }))
                      .concat(isGlobalAdmin ? (whatsappGruposUnidades?.items || []).map((grupo) => ({ id: grupo.canalId, nome: grupo.nomeGrupo })) : [])
                      .filter((grupo, index, arr) => arr.findIndex((item) => String(item.id) === String(grupo.id)) === index)
                    ).map((grupo) => <option key={grupo.id} value={grupo.id}>{grupo.nome}</option>)}
                  </select>
                </label>
                <label>
                  Canal
                  <select value={dashboardGerencialFilters.canalId} onChange={(event) => setDashboardGerencialFilters((current) => ({ ...current, canalId: event.target.value }))}>
                    <option value="todos">Todos</option>
                    {whatsappCanais.map((canal) => <option key={canal.id} value={canal.id}>{canal.nome} Â· {canal.identificador}</option>)}
                  </select>
                </label>
              </div>

              <div className="reportBuilderBox">
                <strong>Montar relatÃ³rio</strong>
              ...
              {dashboardGerencial && (
                <>
                  <div className="managementKpiGrid">
                    <div className="managementKpi"><span>ðŸ’¬</span><strong>{dashboardGerencial.summary.totalMensagens}</strong><small>Mensagens</small></div>
                    <div className="managementKpi"><span>ðŸ‘¤</span><strong>{dashboardGerencial.summary.pacientesUnicos}</strong><small>Pacientes Ãºnicos</small></div>
                    <div className="managementKpi"><span>ðŸ“Œ</span><strong>{dashboardGerencial.summary.agendamentosIniciados}</strong><small>Agendamentos iniciados</small></div>
                    <div className="managementKpi success"><span>âœ…</span><strong>{dashboardGerencial.summary.agendamentosConcluidos}</strong><small>Agendamentos concluÃ­dos</small></div>
                    <div className="managementKpi warning"><span>â†©ï¸</span><strong>{dashboardGerencial.summary.conversasAbandonadas}</strong><small>Abandonadas</small></div>
                    <div className="managementKpi"><span>ðŸ“ˆ</span><strong>{dashboardGerencial.summary.taxaConversao}%</strong><small>ConversÃ£o estimada</small></div>
                  </div>

                  {dashboardReportSections.evolucao && (
                    <div className="dashboardPanel">
                      <h4>EvoluÃ§Ã£o no perÃ­odo</h4>
                        ...
                        {dashboardGerencial.breakdowns.byDay.length ? dashboardGerencial.breakdowns.byDay.map((day) => {
                          ...
                          return (
                            <div className="dayTrendItem" key={day.date}>
                              ...
                              <small>{day.agendamentosConcluidos} concluÃ­do(s)</small>
                            </div>
                          );
                  ...
                  <div className="dashboardSplitGrid">
                    ...
                    {dashboardReportSections.convenios && <TopListPanel title="ConvÃªnios / planos citados" rows={dashboardGerencial.breakdowns.byConvenioPlano} />}
                  ...
                  {dashboardReportSections.logs && (
                    <div className="dashboardPanel dashboardTableWrap">
                      <h4>Logs recentes no relatÃ³rio</h4>
                      <table>
                        <thead><tr><th>Data</th><th>Cliente</th><th>Telefone</th><th>Status</th><th>Etapa</th><th>IntenÃ§Ã£o</th></tr></thead>
                        <tbody>
                ...
                <>
                  <div className="opsHeroGrid">
                    <div className={`opsGaugeCard ${whatsappHealthStats.level}`}>
                      ...
                      <div>
                        ...
                        <span className={`severityBadge ${whatsappHealthStats.level}`}>
                          {whatsappHealthStats.level === "ok" ? "ESTÃƒÂVEL" : whatsappHealthStats.level === "warning" ? "ATENÃ‡ÃƒÆ’O" : "CRÃƒÂTICO"}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="opsKpiGrid">
                      ...
                      <div className={whatsappBlockedNowCount > 0 ? "opsKpiCard critical pulse" : "opsKpiCard ok"}><span>ðŸ›¡ï¸Â</span><strong>{whatsappBlockedNowCount}</strong><small>Bloqueios ativos</small></div>
                      <div className="opsKpiCard info"><span>â±Ã¯Â¸Â</span><strong>{formatDateTimeShort(whatsappDiagnostics.generatedAt)}</strong><small>Ãšltimo diagnÃ³stico</small></div>
                    </div>
                  </div>
                  ...
                  <div className="diagnosticGrid modernDiagnosticGrid">
                    {whatsappDiagnostics.checks.map((check) => (
                      <div key={check.key} className={check.ok ? "diagnosticCard ok" : "diagnosticCard fail pulseSoft"}>
                        <strong>{check.ok ? "âœ…" : "Ã¢Å¡Â Ã¯Â¸Â"} {check.label}</strong>
            ...
            <section className="card groupOverviewPanel operationalGroupPanel">
              <div className="groupOverviewHeader">
                <div>
                  ...
                  <h3>Multi-clÃ­nica agrupada</h3>
                  <p>Resumo dos canais que roteiam um Ãºnico WhatsApp para mÃºltiplas unidades.</p>
                </div>
                <button type="button" className="secondary" onClick={() => loadWhatsappGruposUnidades()} disabled={whatsappGruposLoading}>
                  {whatsappGruposLoading ? "Atualizando..." : "Atualizar grupos"}
                </button>
              ...
              {whatsappGruposUnidades ? (
                <div className="groupOverviewSummary large">
                  <div><strong>{whatsappGruposUnidades.summary.totalGrupos}</strong><span>Grupos</span></div>
                  <div><strong>{whatsappGruposUnidades.summary.unidadesOperacionais}/{whatsappGruposUnidades.summary.totalUnidades}</strong><span>Unidades operacionais</span></div>
                  <div><strong>{whatsappGruposUnidades.summary.gruposCriticos}</strong><span>Grupos crÃ­ticos</span></div>
                  <div><strong>{whatsappGruposUnidades.summary.gruposAtencao}</strong><span>Com atenÃ§Ã£o</span></div>
                </div>
              ) : (
            ...
            <section className="card goLiveChecklistCard">
              <div className="sectionHeader">
                <div>
                  ...
                  <p>ValidaÃ§Ã£o operacional para liberar um cliente/canal para WhatsApp real. Corrija itens crÃ­ticos antes de produÃ§Ã£o.</p>
                </div>
              ...
              {whatsappGoLiveChecklist ? (
                <>
                  <div className="goLiveSummaryGrid">
                    ...
                    <div className="goLiveSummaryCard critical"><strong>{whatsappGoLiveChecklist.summary.critico}</strong><span>CrÃ­ticos</span></div>
                  ...
                  <div className="goLiveFiltersPanel">
                    <div className="goLiveFilterTopline">
                      ...
                      <span>
                        Exibindo {goLiveFilteredSummary.total} de {whatsappGoLiveChecklist.summary.total} canais Â·
                        {" "}{goLiveFilteredSummary.critico} crÃ­tico(s), {goLiveFilteredSummary.atencao} aviso(s), {goLiveFilteredSummary.pronto} pronto(s)
                      </span>
                    </div>
                    <div className="goLiveFiltersGrid">
                      ...
                      <label>
                        ...
                        <select value={goLiveFilters.status} onChange={(event) => setGoLiveFilters({ ...goLiveFilters, status: event.target.value })}>
                          <option value="todos">Todos</option>
                          <option value="critico">CrÃ­ticos</option>
                      ...
                      <label>
                        ...
                        <select value={goLiveFilters.token} onChange={(event) => setGoLiveFilters({ ...goLiveFilters, token: event.target.value })}>
                          ...
                          <option value="proprio">Token prÃ³prio</option>
                  ...
                  <div className="goLivePaginationBar">
                    <span>PÃ¡gina {goLiveSafePage} de {goLivePageCount}</span>
                    <div className="buttonRow compactButtonRow">
                      ...
                      <button type="button" className="secondary" disabled={goLiveSafePage >= goLivePageCount} onClick={() => setGoLivePage((page) => Math.min(goLivePageCount, page + 1))}>PrÃ³xima</button>
                    </div>
                  </div>
                          ...
                          >
                            ...
                            <div>
                              <span className={`severityBadge ${item.status === "pronto" ? "success" : item.status === "atencao" ? "warning" : "critical"}`}>
                                {item.status === "pronto" ? "PRONTO" : item.status === "atencao" ? "ATENÃ‡ÃƒO" : "CRÃTICO"}
                              </span>
                              <h4>{item.clienteNome}</h4>
                              <p>{item.canalNome} Â· {item.phoneNumberId || "sem phone_number_id"} Â· {item.modoAtendimento} Â· {item.ativo ? "canal ativo" : "canal inativo"}</p>
                            </div>
                            <div className="goLiveCompactMeta">
                              <strong>{item.okCount}/{item.totalChecks}</strong>
                              <span>checks OK</span>
                              {problematicChecks.length > 0 ? <small>{problematicChecks.length} pendÃªncia(s)</small> : <small>sem pendÃªncias</small>}
                            </div>
                          </button>
                            ...
                            <div className="goLiveCompactProblems">
                              {problematicChecks.slice(0, 3).map((check) => (
                                <span key={`${item.canalId}-compact-${check.key}`} className={check.severity === "critical" ? "dangerText" : "warningText"}>
                                  {check.severity === "critical" ? "ðŸš¨" : "âš ï¸"} {check.label}
                                </span>
                              ))}
                              {problematicChecks.length > 3 && <span>+{problematicChecks.length - 3} pendÃªncia(s)</span>}
                            </div>
                          )}
                              ...
                              <div className="goLiveChecksGrid">
                                {item.checks.map((check) => (
                                  <div key={`${item.canalId}-${check.key}`} className={`goLiveCheck ${check.ok ? "ok" : check.severity}`}>
                                    <strong>{check.ok ? "âœ…" : check.severity === "critical" ? "ðŸš¨" : "âš ï¸"} {check.label}</strong>
                ...
                <div className="channelTestDashboard">
                  <div className="channelTestHeader">
                    <div>
                      ...
                      <p>SimulaÃ§Ã£o executada sem usar a Meta. Use este painel para validar roteamento, sessÃ£o, token e resposta do Angel.</p>
                    </div>
                  ...
                  <div className="channelTestGrid">
                    <div className="channelTestCard">
                      <span className="channelTestIcon">ðŸ“¡</span>
                      <small>Canal</small>
                  ...
                  <div className="channelTestDetails">
                    <div className="channelTestPanel">
                      ...
                      <dl>
                        ...
                        <div><dt>Origem da intenÃ§Ã£o</dt><dd>{whatsappTestView.source}</dd></div>
                        <div><dt>ConfianÃ§a</dt><dd>{whatsappTestView.confidence}</dd></div>
                  ...
                  {whatsappLogGroups.map((group) => {
                    ...
                    return (
                      <article key={group.key} className={hasErrors ? "conversationGroup criticalConversation" : "conversationGroup"}>
                        ...
                        >
                          <span className="conversationChevron">{expanded ? "â–¾" : "â–¸"}</span>
                          <span className="conversationAvatar">{hasErrors ? "âš ï¸" : "ðŸ’¬"}</span>
                          <span className="conversationMain">
                            <strong>{group.phone}</strong>
                            <span>{group.cliente} Â· canal {group.canal}</span>
                          </span>
                        ...
                        <div className="conversationPreview">
                          <span className="conversationPreviewLabel">Ãšltimo registro</span>
                            ...
                            {orderedLogs.map((log) => {
                              ...
                              return (
                                <div key={`${group.key}-${log.id}`} className={isSystem ? "timelineEvent systemEvent" : "timelineEvent"}>
                                  <div className="timelineMarker">{isSystem ? "ðŸ›¡ï¸" : inbound ? "ðŸ‘¤" : "ðŸ¤–"}</div>
                                  <div className="timelineContent">
                                    ...
                                    <div className="timelineDetails">
                                      ...
                                      {log.intent ? <span>IntenÃ§Ã£o: {log.intent}</span> : null}
